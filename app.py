...
def send():
    if request.method == 'POST':
        try:
            ...
            with sql.connect("database.db") as con:
                cur = con.cursor()
                cur.execute("INSERT INTO reports (data,pc,sala,desc,problema,classificacao,arquivados) VALUES (datetime('now'),?,?,?,?,?,?)",
                            (pc, sala, desc, problema, classificacao, "false"))
                con.commit()
        except:
            con.rollback()
        finally:
...
def update():
    ...
    with sql.connect("database.db") as con:
        cur = con.cursor()
        cur.execute(
            "UPDATE reports SET arquivados = ? WHERE id="+option, ("sim",))
        con.commit()
        ...
        with sql.connect("database.db") as con:
            ...
            cur = con.cursor()
            cur.execute(
                "SELECT * FROM reports WHERE NOT arquivados = ?", ("sim",))
            rows = cur.fetchall()
        ...
        try:
            ...
            if (request.form['actiontype'] == "add"):
                ...
                with sql.connect("database.db") as con:
                    cur = con.cursor()
                    cur.execute(
                        'INSERT INTO salas (nome, elements, total) VALUES (?, ?, ?)', (nome, elements, total))
                    con.commit()
                ...
                with sql.connect("database.db") as con:
                    cur = con.cursor()
                    cur.execute(
                        "UPDATE salas SET elements = ?, total = ? WHERE nome="+nome, (elements, total))
                    con.commit()
